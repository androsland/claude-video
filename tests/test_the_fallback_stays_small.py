...
NON-GOALS, so a green run is not read as more than it is:
  ...
  * **`wv*`/`w` mean "worst by yt-dlp's default sort", not "fewest pixels".**
    That sort puts `res` near the front, so in practice it is resolution — but
    a caller passing `--format-sort` could reorder it. moviola passes none, and
    `test_no_format_sort_is_passed` is what keeps that true; if that test ever
    fails, this file's whole premise needs re-deriving.
  ...
  * **The legitimate configuration this must not fire on is a ladder that HAS
    ...
    must likewise be untouched: `AUDIO_FORMAT` stays `ba/bestaudio`, because
    best audio is that flag's entire point.
...
def _is_height_bounded(atom: str) -> bool:
    return bool(re.search(r"\[height\s*<=?\s*\d+\]", atom))
...
LADDERS: dict[str, list[dict]] = {
    ...
    # No video at all — the chain must select nothing, not raise.
...
def _tallest(ids: tuple[str, ...], formats: list[dict]) -> int:
    by_id = {f["format_id"]: f for f in formats}
    heights = [by_id[i].get("height") for i in ids if by_id[i].get("height")]
...
class TestTheLadderShape:
    ...
    def test_the_audio_only_selector_is_unchanged(self):
        ...
        assert download.AUDIO_FORMAT == "ba/bestaudio"
        for rung in _rungs(download.AUDIO_FORMAT):
...
class TestWhatItSelects:
    ...
    def test_the_fallback_never_grows(self, name):
        ...
        assert _tallest(after, formats) <= _tallest(before, formats), (
            f"on ladder {name!r} the new selector picks {after} "
            f"({_tallest(after, formats)}p), taller than the old {before} "
            f"({_tallest(before, formats)}p)"
        )
    ...
    def test_a_ladder_with_720_is_untouched(self, name):
        ...
        assert _tallest(_select(download.VIDEO_FORMAT, formats), formats) == 720
    ...
    def test_a_ladder_with_no_video_selects_nothing(self):
        # Not an exception: yt-dlp reports the failure itself, and download.py
        # already handles "no video file appeared".
        assert _select(download.VIDEO_FORMAT, LADDERS["audio-only"]) == ()
