...
**Which backend runs:** `--whisper <backend>` wins, then `MOVIOLA_WHISPER` in `~/.config/moviola/.env`, then `auto` — which uses `local` when faster-whisper is importable and falls back to an API key. `auto` never uploads on the strength of an environment variable: with no pin it reads API keys only from `~/.config/moviola/.env` and the working directory's `.env`, so a `GROQ_API_KEY` exported for some other tool cannot cause an upload — and neither can one you exported for moviola, which is indistinguishable and needs a pin like anything else. Set `MOVIOLA_WHISPER=groq` or `openai` (or pass `--whisper`) to trade privacy for speed deliberately.
