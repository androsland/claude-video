...
- `skills/moviola/scripts/untrusted.py` — leaf module holding the structural edits and guarded parses applied to values this program did not write (`LINE_BREAKS`, `balance_bidi`, `stderr_line`, `stderr_block`, `finite_float`). The two stderr shapes are not interchangeable: `stderr_line` makes a short foreign value incapable of ending a line moviola wrote, while `stderr_block` keeps a captured subprocess stderr's structure and prefixes every line of it. Imported by `moviola.py`, `whisper.py`, `local_whisper.py`, `frames.py` and `workdir.py`; it imports nothing of theirs, which is what keeps it out of an import cycle. New fences and new parses of somebody else's output belong here, not in a caller.
