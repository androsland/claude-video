...
- `skills/moviola/scripts/untrusted.py` — leaf module holding the structural edits and guarded parses applied to values this program did not write (`LINE_BREAKS`, `balance_bidi`, `stderr_line`, `finite_float`). Imported by `moviola.py`, `whisper.py`, `local_whisper.py`, `frames.py` and `workdir.py`; it imports nothing of theirs, which is what keeps it out of an import cycle. New fences and new parses of somebody else's output belong here, not in a caller.
